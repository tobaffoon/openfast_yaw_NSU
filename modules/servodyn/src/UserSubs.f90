...
!=======================================================================
SUBROUTINE UserYawCont ( YawPos, YawRate, WindDir, YawError, HorWindV, NumBl, ZTime, DT, DirRoot, YawPosCom, YawRateCom, YawCntDat )
...
REAL(ReKi), INTENT(IN )      :: HorWindV                                        ! Horizontal hub-height wind velocity magnitude [m/s].
...
CHARACTER(1024), INTENT(IN ) :: DirRoot                                         ! The name of the root file including the full path to the current working directory.  This may be useful if you want this routine to write a permanent record of what it does to be stored with the simulation results: the results should be stored in a file whose name (including path) is generated by appending any suitable extension to DirRoot.
!REAL(ReKi), parameter        :: WindDelta = 13.0
!REAL(ReKi), SAVE             :: DummyDelta
REAL(ReKi), parameter        :: Delta = 0.3

write(*, *) YawCntDat%Strategy, YawCntDat%YawErrorDelta, YawCntDat%WindDelta 
if (ABS(YawError) >= Delta) then
    YawRateCom = YawRate
    YawPosCom = YawPos + (WindDir - YawPos) / 10
else
    YawRateCom = 0.0
    YawPosCom = YawPos
end if
!if (HorWindV < WindDelta) then
!    YawRateCom = 0.0
!    YawPosCom = YawPos
!else
